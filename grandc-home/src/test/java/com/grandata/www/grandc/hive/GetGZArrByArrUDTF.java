...
public class GetGZArrByArrUDTF extends GenericUDTF {
  ...
  public void process(Object[] args) throws HiveException {
    ...
    for (int i = 0; i <= p.size() - 4;) {
      try {
        if (Double.parseDouble(p.get(i)) > contract_cap && Double.parseDouble(p.get(i + 1)) > contract_cap
          ...
          endtime = df.format(date_date.getTime() + ((i + 4) * 24 * 60 * 60 * 1000L) / 96);

          Double[] list = {Double.parseDouble(p.get(i)), Double.parseDouble(p.get(i + 1)), Double.parseDouble(p.get(i + 2)), Double.parseDouble(p.get(i + 3))}; // 定义一维数组
  ...
  public static void main(String[] args) throws HiveException {
    GetGZArrByArrUDTF getArrByArrUDTF = new GetGZArrByArrUDTF();

    String list = "0D,1D,2D,3D,4D,5D,6D,7D,8D,9D"
